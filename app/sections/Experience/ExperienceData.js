export const experienceData = [
  {
    date: "June 2019 - August 2019",
    jobTitle: "Design / Marketing Intern",
    location: "Chengdu, China",
    company: "Maxxelli",
    companyLogo: "/logos/logo-maxxelli.svg",
    companyLink: "http://www.maxxelli-consulting.com/",
    tasks: [
      {
        id: "1",
        task: "Created engaging and effective content using CMS platforms Wordpress and WeChat",
      },
      {
        id: "2",
        task: "Designed elegant documents and reports for the company's internal usage",
      },
      {
        id: "3",
        task: "Produced high-quality graphics for social media comms",
      },
    ],
  },
  {
    date: "September 2019 - September 2020",
    jobTitle: "Web Development Intern",
    location: "London, United Kingdom",
    company: "Soapbox",
    companyLogo: "/logos/logo-soapbox.svg",
    companyLink: "https://www.designbysoapbox.com/",
    tasks: [
      {
        id: "1",
        task: "Developed WordPress themes and landing pages using HTML, SCSS, JavaScript and PHP",
      },
      {
        id: "2",
        task: "Ensured smooth running of client websites and plugins by conducting monthly reviews and maintenance checks",
      },
      {
        id: "3",
        task: "Maintained regular communication with clients to gather feedback and ensure their satisfaction with provided services",
      },
    ],
  },
  {
    date: "June 2021 - August 2021",
    jobTitle: "Web Developer & SEO / Web Designer ",
    location: "London, United Kingdom",
    company: "Kinetik Solutions",
    companyLogo: "/logos/logo-kinetik.svg",
    companyLink: "https://kinetik.uk.com/",
    tasks: [
      {
        id: "1",
        task: "Reworked website layouts and behaviour to enhance user experience and improve accessibility, which resulted in improved engagement",
      },
      {
        id: "2",
        task: "Implemented SEO techniques and optimized site performance, leading to faster load times and better website visibility",
      },
      {
        id: "3",
        task: "Created engaging infographics and documents that complemented the company's brand voice and elevated online presence",
      },
    ],
  },
  {
    date: "Sept 2021 - March 2022",
    jobTitle: "Web Developer and Designer",
    location: "London, United Kingdom",
    company: "Selected Group",
    companyLogo: "/logos/logo-selected.svg",
    companyLink: "https://www.selected.org/",
    tasks: [
      {
        id: "1",
        task: "Designed/developed interactive and modern restaurant websites for clients across the UK and Spain",
      },
      {
        id: "2",
        task: "Managed a portfolio of websites with regular updates and maintenance",
      },
      {
        id: "3",
        task: "Worked on an internal tool to manage online reservations and deliveries",
      },
    ],
  },
  {
    date: "April 2022 - Jan 2024",
    jobTitle: "Front End Developer",
    location: "London, United Kingdom",
    company: "Karmoon",
    companyLogo: "/logos/logo-karmoon.svg",
    companyLink: "https://karmoon.co.uk/",
    tasks: [
      {
        id: "1",
        task: "Utilised React, TypeScript and Next.js to compose reactive UIs, write backend API routes and build full stack applications with AI integrations",
      },
      {
        id: "2",
        task: "Developed custom Shopify e-commerce sites and themes using HTML, SCSS, JavaScript, Web components and Liquid",
      },
      {
        id: "3",
        task: "Managed unit tests and optimised projects with Google Lighthouse, third-party software and best practices to increase performance, SEO and accessibility",
      },
      {
        id: "4",
        task: "Regularly communicated with clients to gather feedback and make adjustments throughout the project lifecycle",
      },
    ],
  },
  {
    date: "Feb 2024 - Present",
    jobTitle: "Software Engineer",
    location: "London, United Kingdom",
    company: "Smartzer",
    companyLogo: "/logos/logo-smartzer.svg",
    companyLink: "https://www.smartzer.com",
    tasks: [
      {
        id: "1",
        task: "Developing and maintaining an in-house suite of tools that enable the creation of interactive shopping videos and livestreams. Responsibilites include optimising the player/editor for high traffic, polishing the user experience on desktop/mobile devices as well as making the user flow pleasant, clear and accesible",
      },
      {
        id: "2",
        task: "Working on a monorepo consisting of React, Redux, React Router, Tanstack Query, TypeScript, Node.js, Zod, Firebase and GCP, while also maintaining custom built Shopify apps",
      },
      {
        id: "3",
        task: "Designing new features from ideation and UI/UX through to implementation",
      },
      {
        id: "4",
        task: "Managing the deployment process of new features and bug fixes to production",
      },
      {
        id: "5",
        task: "Participating in customer onboarding and sales calls, as well as daily stand-ups",
      },
    ],
  },
];
